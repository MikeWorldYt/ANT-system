...
import os, sys, platform, string, re, shutil, json, threading, urllib.request
...
def process_yaml_tree(tree_dict: dict, base_path: str, review_index: str, log_fn, path_context: list = None):
...
def build_tree_widget(tree_dict: dict, parent_node, treeview: ttk.Treeview,
    ...
    for key, value in tree_dict.items():
        ...
        node        = treeview.insert(parent_node, "end", text=f"{icon} {key}", open=auto_open)
...
def download_core_files(core_dir: str, log_fn):
    ...
    for item in items:
        ...
        log_fn(f"✔ {core_dir}\\{filename} (downloaded)")
...
class ANTWizard:
    def __init__(self, root):
        self.root = root
        self.root.title("ANT System Setup - New Storage Wizard v.2.2.1")
    ...
    def show_step_1(self):
        ...
        self.next_btn.config(text="Next →", state="normal", command=self.go_next)
            ...
            if not ok:
                ...
                tk.Label(
                    ...
                    text=(f"⚠  Could not load preview\n{error}\n\n"
                          "The tree will be fetched again during creation."),
    ...
    def show_progress_window(self):
        ...
        try:
            ...
            log(f"Wizard online; Generating root directory ...", ACCENT)
            log(f"✔ {root_path}")
            
            # ── Phase 2: Base CFG folders ─────────────────────────
            log(f"\nGenerating a ANT Template ...", ACCENT)
            ...
            # ── Phase 5: Core utility files ───────────────────────
            log(f"\nDownloading core utility files ...", ACCENT)
            ...
            try:
                ...
                log(f"✖ Failed to download README: {e}", ACCENT)
            ...
            try:
                ...
                messagebox.showerror(
                    "Error",
                    f"Failed to download core utility files:\n{e}\n\n"
                    "Please download them manually from the repository:\n"
                    f"https://MikeWorldYt/ANT-system/contents/Migration/Core\n"
                    f"and place them in:\n"
                    f"  {core_dir}\n\n"
                )
            ...
            tk.Button(
                ...
                font=("Segoe UI", 10, "bold"),
                bg=BUTTON, fg=TEXT,
                activebackground="#00a077", activeforeground=TEXT,
